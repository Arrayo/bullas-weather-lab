...
const [forecastInfo] = await queryAll<{ latest_downloaded_at: string | null; count: number }>(app.db, sql`SELECT MAX(downloaded_at) AS latest_downloaded_at, COUNT(*) AS count FROM hourly_forecasts WHERE station_id = ${station.id}`);
const [obsInfo] = await queryAll<{ latest_observed_at: string | null; count: number }>(app.db, sql`SELECT MAX(observed_at) AS latest_observed_at, COUNT(*) AS count FROM hourly_observations WHERE station_id = ${station.id}`);
const [abandonedForecasts] = await queryAll<{ count: number }>(app.db, sql`SELECT COUNT(*) AS count FROM forecast_collections WHERE station_id = ${station.id} AND finished_at IS NULL`);
const abandonedRuns = await app.dataCollectionRepository.countRunning(station.id);
const verification = await app.forecastRepository.verifyAgainstObservations(station.id, { minimumLeadMinutes: app.config.FORECAST_MINIMUM_LEAD_MINUTES });

const forecastAgeHours = forecastInfo?.latest_downloaded_at ? (now.getTime() - new Date(forecastInfo.latest_downloaded_at).getTime()) / 3_600_000 : null;
const observationAgeHours = obsInfo?.latest_observed_at ? (now.getTime() - new Date(obsInfo.latest_observed_at).getTime()) / 3_600_000 : null;
...
const stale = forecastAgeHours === null || observationAgeHours === null || forecastAgeHours > app.config.FORECAST_STALE_AFTER_HOURS || observationAgeHours > app.config.OBSERVATION_STALE_AFTER_HOURS;
const failed = !hasUsableData || latestRun?.status === "failure";
const degraded = (latestRun?.requiredModelsFailed.length ?? 0) > 0 || abandonedRuns > 0 || (abandonedForecasts?.count ?? 0) > 0;
...
const output = {
  ...
  station: station.id,
  status,
  latestRun: latestRun ? { id: latestRun.id, status: latestRun.status, startedAt: latestRun.startedAt.toISOString(), finishedAt: latestRun.finishedAt?.toISOString() ?? null, ageHours: latestRunAgeHours } : null,
  ...
  validVerifications: verification.rows.length,
...
if (json) writeJson(output);
  ...
  if (output.optionalModelsFailed.length > 0) console.log(`Optional model unavailable: ${output.optionalModelsFailed.join(", ")}`);
  ...
  console.log(`Verificaciones temporalmente válidas: ${output.validVerifications}`);
